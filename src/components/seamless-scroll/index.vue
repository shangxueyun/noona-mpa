<template>
  <div ref="wrap">
    <div :style="leftSwitch" v-if="isHorizontal" :class="leftSwitchClass" @click="isExtsisEvent ? leftSwitchClick : ''">
      <slot name="left-switch" />
    </div>
    <div :style="rightSwitch" v-if="isHorizontal" :class="rightSwitchClass" @click="isExtsisEvent ? rightSwitchClick : ''">
      <slot name="right-switch" />
    </div>
    <template v-if="isExtsisEvent">
      <div
        ref="realBox"
        :style="pos"
        @mouseenter="enter"
...
import { animationFrame, arrayEqual, copyObj } from './utils'
animationFrame()
export default {
  ...
  computed: {
    ...
    baseFontSize () {
      return this.options.isSingleRemUnit ? parseFloat(window.getComputedStyle(document.documentElement, null).fontSize) : 1
    },
    realSingleStopWidth () {
    ...
    _move () {
      ...
      this.reqFrame = requestAnimationFrame(
        function () {
          ...
          if (this.singleWaitTime) clearTimeout(this.singleWaitTime)
          let isRealSingleStopWidth = !!this.realSingleStopWidth
          let isRealSingleStopHeight = !!this.realSingleStopHeight
          if (isRealSingleStopHeight) { // 是否启动了单行暂停配置
