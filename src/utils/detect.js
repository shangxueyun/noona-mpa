...
const detect = function () {
  ...
  let android = u.match(/(Android);?[\s\\/]+([\d.]+)?/)
  let iPad = u.match(/(iPad).*OS\s([\d_]+)/)
  let iPod = u.match(/(iPod)(.*OS\s([\d_]+))?/)
  let iPhone = !iPad && u.match(/(iPhone\sOS)\s([\d_]+)/)
  let chrome = u.match(/Chrome\/([\d.]+)/) || u.match(/CriOS\/([\d.]+)/)
